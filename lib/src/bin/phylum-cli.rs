...
/// details.
fn handle_auth_register(api: &mut PhylumApi, config: &mut Config, config_path: &str) {
    ...
    print_user_success!("Successfully registred a new account!");
}
...
fn handle_auth_login(api: &mut PhylumApi, config: &mut Config, config_path: &str) {
    let email: String = Input::with_theme(&ColorfulTheme::default())
        .with_prompt("Email address")
        .validate_with({
            move |email: &String| -> Result<(), &str> {
    ...
    print_user_success!("Successfully authenticated with Phylum");
}
...
fn handle_auth(
    ...
    if matches.subcommand_matches("register").is_some() {
        handle_auth_register(api, config, config_path);
    } else if matches.subcommand_matches("login").is_some() {
        handle_auth_login(api, config, config_path);
...
fn download_file(latest: &GithubReleaseAsset, dest: &str) -> Option<usize> {
    get_github(latest.browser_download_url.as_str(), |r| -> Option<usize> {
        ...
        let mut file = std::fs::File::create(dest)
            .expect("Failed to create temporary update file");
...
fn update_in_place(latest: GithubRelease) -> Result<String, std::io::Error> {
    ...
    // TODO: Should update this to check for the `phylum` binary. The name of
    //       the binary was changed.
    let bin_asset = &latest.assets
        .iter()
        .find(|x| x.name == "phylum-cli")
        .unwrap();

    let bash_asset = &latest.assets
        .iter()
    ...
    match fs::set_permissions(&current_bin, fs::Permissions::from_mode(0o770)) {
        Ok(_) => {},
        Err(_) => {
            print_user_warning!("Successfully downloaded updates, but failed to make binary executable");
        }
    };
...
fn main() {
    ...
    // TODO: Need to actually check the `ver` from above.
    let latest_version = get_latest_version();
    if matches.subcommand_matches("update").is_none()
        && needs_update(&latest_version, ver) {
