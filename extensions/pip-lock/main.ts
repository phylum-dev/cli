...
async function checkAndRunPip(args, pipPath: string): Promise<PackageList> {
    ...
    const proc = Deno.run({
        cmd: [pipPath, 'install', '-r', args, '--dry-run'],
        stdout: 'piped',
        stderr: 'piped'
    });
...
//Output strict requirements.txt
function outputStrict(packageList: [object]) {
    let strictList = [];
    for(const element of packageList.packages) {
	strictList.push(`${element.name}==${element.version}`);
    }
    console.log(strictList.join("\n"));
}
...
//Parse arguments
const parsedArgs = parse(Deno.args, {"boolean": true});

// Perform pip dry run install to gather relevant package list.
const packageList = await checkAndRunPip(parsedArgs._[0], pipPath);

//Check for "--analyze" option, submit if present, output in requirements.txt format if not
if(parsedArgs.analyze) {
    const job = await PhylumApi.analyze('pypi', packageList.packages);
    const jobStatus = await PhylumApi.getJobStatus(job);
    console.log(jobStatus);
} else {
    outputStrict(packageList);
}
